...
public class ClerkFrame extends JFrame {
    ...
    //Process Shipment Request panel, displays all the shipments of the customer and allows  the clerk to change delivery status and payment status
    ...
    private JPanel assignPackagesPanel() {
        ...
        String[] columns = {"Tracking #", "Recipient", "Weight (kg)", "Type", "Zone", 
                           "Status", "Cost", "Payment", "Assign Driver", "Assign Vehicle", "Assign Route"};
        
        DefaultTableModel model = new DefaultTableModel(columns, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                // Status, Payment, and assignment columns are editable
                return column == 5 || column == 7 || column == 8 || column == 9 || column == 10;
            }
        };
        ...
        if (driversList != null) {
            for (Map<String, String> driver : driversList) {
                String displayName = driver.get("username") + " (ID: " + driver.get("driverId") + 
                                   ", Deliveries: " + driver.get("totalDeliveries") + ")";
        ...
        if (vehiclesList != null) {
            for (Map<String, String> vehicle : vehiclesList) {
                String displayName = vehicle.get("licensePlate") + " - " + vehicle.get("vehicleType") +
                                   " (Capacity: " + vehicle.get("capacity") + " kg, Items: " + 
                                   vehicle.get("currentItemCount") + ")";
        ...
        // Routes combo box (hardcoded for now since no routes table exists)
        String[] routes = {" Select Route ", "Route A - North", "Route B - South", 
                          "Route C - East", "Route D - West", "Route E - Central"};
        ...
        shipmentsTable.getModel().addTableModelListener(e -> {
                ...
                if (driverId != null) {
                    ...
                    if (success) {
                        JOptionPane.showMessageDialog(null, 
                            "Driver assigned successfully to tracking #" + tracking,
                            "Success", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(null, 
                            "Failed to assign driver to tracking #" + tracking,
            ...
            if (col == 9) {
                ...
                if (vehicleId != null) {
                    ...
                    if (success) {
                        JOptionPane.showMessageDialog(null, 
                            "Vehicle assigned successfully to tracking #" + tracking,
                            "Success", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(null, 
                            "Failed to assign vehicle to tracking #" + tracking,
                            "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
            ...
            if (col == 10) {
                ...
                if (!selectedRoute.equals("-- Select Route --")) {
                    ...
                    if (success) {
                        JOptionPane.showMessageDialog(null, 
                            "Route assigned successfully to tracking #" + tracking,
                            "Success", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(null, 
                            "Failed to assign route to tracking #" + tracking,
                            "Error", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
    ...
    /**
     * Helper method to load shipment data with assignments
     */
    private void loadShipmentData(DefaultTableModel model) {
        ...
        if (shipments != null) {
            for (Map<String, String> shipment : shipments) {
                ...
                Map<String, String> assignment = assignments != null ? 
                    assignments.get(trackingNumber) : null;
                ...
                if (assignment != null) {
                    if (assignment.containsKey("driverName")) {
                        driverDisplay = assignment.get("driverName") + " (ID: " + 
                                      assignment.get("driverId") + ")";
                    }
                    if (assignment.containsKey("licensePlate")) {
                        vehicleDisplay = assignment.get("licensePlate") + " (ID: " + 
                                       assignment.get("vehicleId") + ")";
                    }
                    if (assignment.containsKey("route")) {
